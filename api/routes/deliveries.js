    ...
    for (let i = 0; i < orders.length; i++) {
        ...
        if (res.rows.length == 0) {
                ...
                try {
                    ...
                    await pool.query('INSERT INTO master_data (reference_' + sellerCompany.id + ', reference_' + buyerCompany.id + ', category) VALUES ($1, $2, $3)', [deliveryOrderId, goodsReceiptId, "Document"], (error, result) => {
                        if (error) {
                            ...
                            console.log(deliveryOrderId + ' - Doesnt exist, goods receipt was created on company ' + buyerCompany.id + ' with id: ' + goodsReceiptId)
                        }
                    });
                } catch (err) {
                    console.log(err);
            ...
            if (res.rows.length == 1)
                console.log(deliveryOrderId + ' - Already exists with id on company ' + buyerCompany.id + ' being: ' + res.rows[0])
            else
                console.log(deliveryOrderId + ' - Error with order check')
        }
    }
